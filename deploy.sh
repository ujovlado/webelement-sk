...
docker run -i -t --rm \
  ...
  && docker tag webelement/webelement-web "$AWS_ACCOUNT_ID.dkr.ecr.eu-central-1.amazonaws.com/webelement/web:latest" \
  && docker push "$AWS_ACCOUNT_ID.dkr.ecr.eu-central-1.amazonaws.com/webelement/web:latest"
